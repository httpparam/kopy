...
RETURNS TABLE(id TEXT, encrypted_content TEXT, sender_name TEXT, password_hash TEXT, created_at TIMESTAMP WITH TIME ZONE, expires_at TIMESTAMP WITH TIME ZONE) AS $$
BEGIN
  ...
  RETURN QUERY
  SELECT p.id, p.encrypted_content, p.sender_name, p.password_hash, p.created_at, p.expires_at
  FROM pastes p
